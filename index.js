...
}
{
    ...
    $(document).ready(function() {
        $('.navbar__burger').click(function(event) {
            $('.navbar__burger,.navbar__content').toggleClass('active');
        ...
        $('ul.sub-menu').wrap('<div class="sub-menu-wrap" />');

        $('.spoiler__title').click(function(event) {
            $(this).toggleClass('active').next().slideToggle(300);
        });

        $('.testimonials__desc').each(function() {
            var $this = $(this);
            var text = $this.html();
            $this.data('full-text', text);
            var small = text.substring(0, 134);

            if (small.length < text.length) {
                $this.html(small);
                var $button = $('<a href="#fulltext" class="testimonials__readmore">Читать весь отзыв</a>');
...
/*
playback timings (ms):
  captures_list: 170.665
  exclusion.robots: 0.089
  exclusion.robots.policy: 0.079
  cdx.remote: 0.07
  esindex: 0.009
  LoadShardBlock: 138.826 (3)
  PetaboxLoader3.datanode: 38.91 (5)
  PetaboxLoader3.resolve: 140.082 (3)
  load_resource: 81.526
  loaddict: 38.887
*/x
